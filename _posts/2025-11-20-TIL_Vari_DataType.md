...
- 의문 3
  - C#에서 int형 데이터를 계산할 때 잠시 float로 형변환 하는 거랑, 처음부터 float형 데이터로 선언하고 계산하는 거랑 메모리나 연산 속도에 유의미한 차이가 있을까?
  - 확인한 사항 : 둘 다 값타입이고 4바이트라서 유의미한 차이가 없음, 값타입과 참조타입의 차이라면 연산 차이가 클 것
