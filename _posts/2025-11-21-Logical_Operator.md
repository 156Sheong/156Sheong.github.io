...
조건부 OR                              & ||                                  & \begin{tabular}[c]{@{}c@{}}두   개의 피연산자 중 하나가 TRUE면 TRUE로 반환,\\      두 피연산자 모두 FALSE면 FALSE로 반환\end{tabular}                          & 왼쪽   피연산자 먼저 평가 -\textgreater 값이 TRUE 이면 우측 피연산자는 평가 건너뜀  &  \\
비조건부 OR                             & |                                   & \begin{tabular}[c]{@{}c@{}}두   개의 피연산자 중 하나가 TRUE면 TRUE로 반환,\\      두 피연산자 모두 FALSE면 FALSE로 반환\end{tabular}                          & X                                                         &  \\
배타적 논리합 XOR                         & \textasciicircum{}                  & \begin{tabular}[c]{@{}c@{}}두   개의 피연산자의 값이 서로 다를 경우에만 TRUE로 반환,\\      두피연산자의 값이 서로 같을 경우 FALSE로 반환\end{tabular}                     & X                                                         &  \\
논리 부정 NOT                           & !                                   & \begin{tabular}[c]{@{}c@{}}단항   연산자로, 피연산자의 논리 값을 반대로 뒤집음,\\      !TRUE -\textgreater FALSE / !FALSE -\textgreater TRUE\end{tabular} & X                                                         & 
\end{tabular}
\end{table}